...
public class MainFrame extends JFrame {
    protected final ButtonGroup menuTools;
    protected final ButtonGroup menuColors;
    protected final Map<String, JRadioButtonMenuItem> menuToolsMap;
    protected final Map<String, JRadioButtonMenuItem> menuColorsMap;
    protected final ButtonGroup menuToolbarTools;
    protected final ButtonGroup menuToolbarColors;
    protected final Map<String, JToggleButton> menuToolbarToolsMap;
    protected final Map<String, JToggleButton> menuToolbarColorsMap;
    ...
    public MainFrame() {
        ...
        menuTools = new ButtonGroup();
        menuToolsMap = new HashMap<>();

        menuToolbarTools = new ButtonGroup();
        menuToolbarToolsMap = new HashMap<>();

        menuColors = new ButtonGroup();
        menuColorsMap = new HashMap<>();

        menuToolbarColors = new ButtonGroup();
        menuToolbarColorsMap = new HashMap<>();
    ...
    public void addRadioMenuItem(String title, String tooltip, int mnemonic, String icon, String actionMethod) throws SecurityException, NoSuchMethodException {
        ...
        if (element instanceof JMenu) {
            ((JMenu) element).add(item);
            if (isToolsMenuItem(title)) {
                menuToolsMap.put(title, item);
                menuTools.add(item);
            } else {
                menuColorsMap.put(title, item);
                menuColors.add(item);
            }
    ...
    private boolean isToolsMenuItem(String menuPath) {
        return (menuPath.substring(0, menuPath.indexOf("/"))).equals("Tools");
    }
    ...
    public void addToolBarToggleButton(String menuPath) {
        ...
        toolBar.add(button);
        if (isToolsMenuItem(menuPath)) {
            menuToolbarTools.add(button);
            menuToolbarToolsMap.put(menuPath, button);
        } else {
            menuToolbarColors.add(button);
            menuToolbarColorsMap.put(menuPath, button);
        }
    }
